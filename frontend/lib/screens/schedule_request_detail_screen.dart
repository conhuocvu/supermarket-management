...
class ScheduleRequestDetailScreen extends StatelessWidget {
  final RequestItem request;

  const ScheduleRequestDetailScreen({Key? key, required this.request}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    final currentShiftDate = request.details['currentShiftDate'] ?? '';
    final currentShiftTime = request.details['currentShiftTime'] ?? '';
    final targetShiftDate = request.details['targetShiftDate'] ?? '';
    final targetShiftTime = request.details['targetShiftTime'] ?? '';
    final colleague = request.details['colleague'] ?? '';
            ...
            BentoCard(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  _buildDetailRow(context, 'Partner Colleague', colleague),
                  const Divider(),
                  _buildDetailRow(context, 'Your Shift Date', currentShiftDate),
                  _buildDetailRow(context, 'Your Shift Hours', currentShiftTime),
                  const Divider(),
                  _buildDetailRow(context, 'Target Shift Date', targetShiftDate),
                  _buildDetailRow(context, 'Target Shift Hours', targetShiftTime),
                  const Divider(),
  ...
  void _updateRequestStatus(BuildContext context, AppState appState, RequestStatus status) {
    ...
    if (index != -1) {
        ...
        timeline: [
          ...
          TimelineEvent(
            ...
            description: status == RequestStatus.approved
                ? 'Shift swap approved by Sarah Jenkins'
                : 'Shift swap rejected by Sarah Jenkins',
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Shift Swap Request has been ${status == RequestStatus.approved ? "Approved" : "Rejected"}'),
